...
def SMALT_models_low(parameter):
	...
	#adv_rate = pow(10,-4)
	adv_rate = -parameter["adv_rate"]
	adv_rate = pow(10,adv_rate)
...
####reference:https://stackoverflow.com/questions/44572109/what-are-the-arguments-for-scipy-stats-uniform
...
parameter_prior = Distribution(
	#mu = RV("uniform", 3, 1), birth_rate = RV("uniform", 0.15, 0.4), s_coef = RV("uniform", 0, 3)
	#mu = RV("uniform", 3, 1), birth_rate = RV("uniform", 0.15, 0.4), s_coef = RV("uniform", 0, 0.1)
	mu = RV("uniform", 3, 1), birth_rate = RV("uniform", 0.15, 0.4), s_coef = RV("uniform", 0, 0.1),adv_rate=RV("uniform", 3,2)
	)
...
#abc = ABCSMC(SMALT_models_low, parameter_prior, distance, population_size = nsim,sampler=sampler.SingleCoreSampler(), eps=QuantileEpsilon(alpha=alphavaule))
...
#abc = ABCSMC(SMALT_models, parameter_priors, distance, population_size = nsim, eps=QuantileEpsilon(alpha=alphavaule), sampler = sampler.SingleCoreSampler())
#abc = ABCSMC(SMALT_models, parameter_priors, distance, population_size = nsim, sampler = sampler.SingleCoreSampler())
#db_title = "".join(["/db/",str(inputRdata),".db"])
db_path = create_sqlite_db_id(file_= f"./{dataname}_single_model_nsim={nsim}_npopulations={npopulation}_{processID}_nohotspot_no75_4para.db")
...
dfm_para = pd.DataFrame()
for t in range(history.max_t + 1):
	df, w = history.get_distribution(m=0, t=t)
	df['t'] = t
	dfm_para = pd.concat([df, dfm_para],axis=0,ignore_index=True)
...
#ax.axvline(sum_stat_obs, color="k", linestyle="dashed")
#ax.axvline(sum_stat_obs, color="k", linestyle="dashed")
ax[0].legend(ncol=10,loc=9, bbox_to_anchor=(0,1.2),borderaxespad=0.)
dfm_para.to_csv(f'./{dataname}_single_model_nsim={nsim}_npopulations={npopulation}_{processID}_parameter_wide.csv', sep="\t", index=False, na_rep='NA')
dfm_para = dfm_para.melt(id_vars=['t'],var_name='Parameter')
dfm_para.to_csv(f'./{dataname}_single_model_nsim={nsim}_npopulations={npopulation}_{processID}_parameter.csv_long', sep="\t", index=False, na_rep='NA')
fig.savefig(f"./{dataname}_single_model_nsim={nsim}_npopulations={npopulation}_{processID}.pdf",bbox_inches = 'tight')
